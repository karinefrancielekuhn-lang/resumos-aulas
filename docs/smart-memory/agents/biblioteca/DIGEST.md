---
title: "DIGEST — biblioteca (Sorae)"
kind: digest
type: overview
status: active
updated: 2026-09-02
tags: [digest, biblioteca]
---

# DIGEST — biblioteca (Sorae / edu-bibliotecario)

> Resumo vivo da área. Leitura obrigatória para o edu-bibliotecario além do INDEX e stories ativas.

## Estado atual

- `_INDEX - Aulas.md` (raiz do projeto) é o índice geral — mantido por Sorae, atualizar
  a cada aula processada.
- `0 - Copywriting/_SINTESE-CONSOLIDADA.md` — compilado de copy, migrado da aula
  Bifi+Amanda. Só uma fonte até agora; crescerá conforme novas aulas alimentarem copy.
- `Guias/` tem 2 guias migrados (GUIA-CELULAR-ZERO, GUIA_OPERACIONAL_VENDAS_ORGANICAS).
- `Materiais/` (dentro de cada Estratégia) ainda vazias — aguardando o usuário colar
  materiais das aulas.
- **Migração pendente:** `_Projeto-Antigo/` ainda existe como staging/backup — todo
  conteúdo relevante já foi movido, mas a pasta não foi apagada (ver decisions/).

## Referências vivas (kind: reference)

| Nota | O que é |
|---|---|
| [[../../decisions/2026-09-02-migracao-taxonomia|Migração de taxonomia]] | Mapeamento completo tema→Estratégia |
| [[../../../../../_INDEX - Aulas|_INDEX - Aulas]] | Índice geral (autoridade exclusiva da Sorae) |

## Episódios

| Episódio | Status | Conclusão (summary) |
|---|---|---|
| Migração inicial (2026-09-02) | resolved | Taxonomia reestruturada por Estratégia; _INDEX - Aulas criado; guias movidos |
